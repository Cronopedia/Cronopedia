package br.com.cronopedia.paginasapi.security;
...
import br.com.cronopedia.paginasapi.security.jwt.AuthenticationJwtFilter;
import br.com.cronopedia.paginasapi.security.jwt.JwtLoginFilter;
import br.com.cronopedia.paginasapi.security.jwt.JwtUtils;
import br.com.cronopedia.paginasapi.security.services.UserDetailsServiceImpl;
...
public class AppSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    // UserCredentials user1 = User.withUsername("user1")
    ...
    // UsernamePasswordAuthenticationFilter.class);
    // // Direção: Client -> Server. Executado quando um Client vai fazer login na
    // nossa aplicação.
    // // Ex: Post /login/auth -> Esse filtro então irá receber o ususario e senha e
    // verificar se ele pode acessar a aplicação
    ...
    // // new JwtLoginFilter("/login", authenticationManager(new
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().configurationSource(corsConfigurationSource())
                ...
                .antMatchers(HttpMethod.POST, "/user").permitAll()
                .antMatchers(HttpMethod.POST, "/login").permitAll()
                .antMatchers(HttpMethod.GET, "/paginas/**").permitAll()
                .antMatchers(HttpMethod.OPTIONS, "/paginas/add/**").authenticated()
                .antMatchers(HttpMethod.OPTIONS, "/paginas/addArray/**").authenticated()
                .antMatchers(HttpMethod.OPTIONS, "/paginas/deletar/**").authenticated()
                .antMatchers(HttpMethod.OPTIONS, "/paginas/atualizar/**").authenticated()
                .antMatchers(HttpMethod.GET, "/user/**").permitAll() // Perfil de outro
                .antMatchers(HttpMethod.OPTIONS, "/me").authenticated() // perfil do usuário
                .anyRequest().authenticated()
                .and()
    ...
    // return new JwtLoginFilter("/login",
    // http.getSharedObject(AuthenticationManager.class));
    // }
}
