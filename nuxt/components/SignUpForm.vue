<template>
    ...
    <form action="" class="login-form sign" @submit.prevent="">
        ...
        <input type="text" name="nome" id="nome" class="input-style-1" placeholder="Nome" v-on:input="validaNome()" v-model="user.nome">
        <small class="mensagem-erro nome"> Nome vazio </small>

        <label for="email" id="email-label" class="label-login">
            <h4>Email</h4>
        </label>
        <input type="email" name="email" id="email" class="input-style-1" placeholder="exemplo@gmail.com"
            v-on:input="validaEmail()" v-model="user.email">
        <small class="mensagem-erro email"> Email vazio </small>

        <label for="senha" id="password-label" class="label-login">
            <h4>Senha</h4>
        </label>
        <input type="password" name="senha" id="senha" class="input-style-1" placeholder="Insira sua senha"
            v-on:input="validaSenha()" v-model="user.senha">
        <small class="mensagem-erro senha"> Senha vazia </small>

        <label for="nickname" id="nickname-label" class="label-login">
            <h4>Nickname</h4>
        </label>
        <input type="text" name="nickname" id="nickname" class="input-style-1" placeholder="Nome de usuário"
            v-on:input="validaNick()" v-model="user.nickname">
        <small class="mensagem-erro nickname"> Nickname vazio </small>

        <label for="data" id="data-label" class="label-login">
            <h4>Data de Nascimento</h4>
        </label>
        <input type="date" name="data" id="data" class="input-style-1" placeholder="Data" v-on:input="validaData()" v-model="user.dataNascimento">
        <small class="mensagem-erro data"> Data vazia </small>

        <button type="submit" class="primary sign-up" v-on:click="validar()"></button>
    </form>
...
export default {
    ...
    methods: {
        ...
        validar() {
            ...
            par1 && par2 ? (alert("Sucesso! ")) : alert("Algum dos campos está vazio!");
